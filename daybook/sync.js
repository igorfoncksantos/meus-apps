...
(function(){
  ...
  function injectCSS(){ if(document.getElementById('syncCss'))return; var st=document.createElement('style'); st.id='syncCss';
    ...
    '#syncBtn{position:fixed;top:calc(10px + env(safe-area-inset-top));right:12px;z-index:9998;width:38px;height:38px;border-radius:12px;border:1px solid var(--line,rgba(255,255,255,.14));background:var(--panel,rgba(20,23,27,.82));backdrop-filter:blur(8px);color:var(--mut,#cfd6db);display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,.4)}#syncBtn.on{color:'+ACCENT+';border-color:rgba(47,217,201,.4)}#syncBtn svg{width:19px;height:19px}'+
  ...
  var IC={bio:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12C2 6.5 6.5 2 12 2a10 10 0 0 1 8 4"/><path d="M5 19.5C5.5 18 6 15 6 12c0-.7.12-1.37.34-2"/><path d="M17.29 21.02c.12-.6.43-2.3.5-3.02"/><path d="M12 10a2 2 0 0 0-2 2c0 1.02-.1 2.51-.26 4"/><path d="M8.65 22c.21-.66.45-1.32.57-2"/><path d="M14 13.12c0 2.38 0 6.38-1 8.88"/><path d="M21.8 16c.2-2 .13-5.35 0-6"/><path d="M9 6.8a6 6 0 0 1 9 5.2c0 .47 0 1.17-.02 2"/></svg>',cfg:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3.2"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>',sync:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>',
  ...
  function _tecEstado(){
    try{ return localStorage.getItem("apps-teclado") || "auto"; }catch(e){ return "auto"; }
  }
  function _tecBotao(){
    var e = _tecEstado();
    var rot = (e === "sim") ? "sempre" : (e === "nao") ? "nunca" : "automático";
    return '<button onclick="window.__tec()">' + IC.pen + ' Teclado do app: ' + rot + '</button>';
  }
  /* gira entre os tres e recarrega: o teclado se monta na abertura */
  window.__tec = function(){
    var e = _tecEstado();
    var prox = (e === "auto") ? "sim" : (e === "sim") ? "nao" : "auto";
    try{ localStorage.setItem("apps-teclado", prox); }catch(x){}
    location.reload();
  };
  function _somBotao(){
  ...
  /* ---------- botão de acesso ---------- */
  function updateBtn(){ var b=document.getElementById('syncBtn'); if(!b)return; b.classList.toggle('on', ready()||!!localStorage.getItem(LK.login)); b.innerHTML=IC.cfg; }
